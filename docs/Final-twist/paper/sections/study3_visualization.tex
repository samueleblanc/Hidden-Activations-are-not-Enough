...
section reports, with equal prominence, two places where it does
\emph{not} buy what one might hope, and one methodological commitment that
constrains what we allow ourselves to report. The scope of each negative is
...
(\emph{region\_ok} $=0/54$, Limitation~L4).}
...
(Section~\ref{sec:limitations}, L4).

\subsection{Knowledge-matrix drift is measured, never asserted from a theorem}
\label{sec:study3-teleport}

\paragraph{Why it matters.}
The third item is not a negative result but a methodological commitment, and it
is the one that makes the other two credible. The temptation in Appendix~\ref{app:teleport-checks}
(Section~\ref{sec:study1b}) is to enter a table zero for the knowledge-matrix
drift on the strength of the invariance theorem, since teleportation is an exact
quiver isomorphism and the theorem says the drift vanishes. We do not do that,
for a reason that survives the transform being exact: by
Theorem~\ref{thm:nogo} there is \emph{no} bound on knowledge-matrix germ drift
from function-space ($C^0$) closeness alone --- two networks can be uniformly
arbitrarily close as functions yet have knowledge matrices arbitrarily far apart
at a point. A theorem-asserted zero would therefore be reporting a number the
experiment did not produce, and would conceal exactly the quantity a sceptical
reader wants to see: how far the implementation actually sits from the identity
it is supposed to satisfy.

\paragraph{Reading.}
We measure both components instead. The visible one is checked per pair through
the equality of Proposition~\ref{prop:visdrift}(ii); the invisible one --- the
part Theorem~\ref{thm:nogo} shows the logits cannot constrain --- is computed
directly at double precision, giving $0.8$--$2.5\times10^{-15}$ relative on all
three architectures (Appendix~\ref{app:teleport-km}). The same discipline
governs the numbers reported elsewhere in this paper: the permutation residual of
the permutation check is quoted as float32 accumulation rather than rounded to zero, and the
float32 completeness residual is quoted at $0.15$--$0.30$ on ResNet-152 rather
than described as machine-precision agreement. Where the theory predicts an
identity, we report the distance from it.
