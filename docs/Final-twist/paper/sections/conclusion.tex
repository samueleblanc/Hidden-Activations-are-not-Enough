...
Section~\ref{sec:study1}), but function-equivalence holds only
approximately --- the logit drift reaches $\sim10^{-1}$ on ResNet-152 and
stays under $4\times10^{-2}$ on the other two --- so the
knowledge matrix's visible drift is gated by the row-sum identity ---
reported as \emph{equal} to that logit gate rather than asserted zero
(Proposition~\ref{prop:visdrift}, Limitation L1).
...
adversarial pairs and reads the resulting coherence $A=(d_f/d_M)^2$
...
(\texttt{region\_ok} = 0/54, Limitation L3).
...
suggest themselves from the Limitations of this paper. A BatchNorm-aware
extension of
\texttt{neuralteleportation}'s change-of-basis transformation would
tighten the function-equivalence gate in Study~1B from the current
$2$--$10\%$ logit drift toward the floating-point floor, so that the
measured knowledge-matrix gate shrinks to accumulation noise (closes L1).
A concat-aware extension of the random-permutation correctness check
would extend Study~1A from ResNet-152 to all three architectures (closes
L4). A multi-region or continuation reformulation of the
$\M(x)$-counterfactual would convert the structural negative of
Section~\ref{sec:study3} into a positive one (closes L3). Concrete
