...
$\M(x)\mathbf{1}=\Netx$ hold exactly. Throughout, $\M(x)\in\R^{C\times(d+1)}$ is
...
$\Weff(x)=J=\partial\Net/\partial x\in\R^{C\times d}$ and its final column is
the aggregate bias attribution $\beff(x)=\Netx-Jx$.
...
Because $\Weff(x)=J=\partial\Net/\partial x$ is exactly the Jacobian of the
...
additional forward pass gives $\Netx$, from which the bias column is
$\beff(x)=\Netx-Jx$. This is $C$ backward passes plus one forward pass.

By Theorem~\ref{thm:weff} these three routes coincide at almost every input for
networks satisfying (LCS): they agree at every regular point, i.e.\ off the measure-zero set
...
properties of $\M$ under (LCS) are shared with that data, and what the matrix
...
$\Weff(x)=J=\partial\Net/\partial x \in \R^{C\times d}$ (the knowledge matrix
...
which the row-sum identity $\M(x)\mathbf{1}=\Netx$ is exact. Per
Theorem~\ref{thm:weff}, every function-level property of $\M$ under (LCS)
...
residual $\|\M(x)\mathbf{1}-\Netx\|_\infty = 2.0\times10^{-14}$, and the
$\Weff$ identity $\|[\,\Weff\mid c\,]\,\mathbf{1}-\Netx\|_\infty
= 2.1\times10^{-14}$.
...
perturbation on the same network gives $d_M = d_\Psi$ to
...
pattern, and the row-sum identity $\M(x)\mathbf{1}=\Netx$ then appears to fail at
