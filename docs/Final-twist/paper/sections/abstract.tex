...
feedforward network, an exact row-sum $\Mfx\,\mathbf 1 = \Netx$
\clarify{(throughout, $f$ is the \emph{activation} function and $W$ the weight
collection, so that $\Net$ denotes the network function they realise --- the
same $W$ and the same $f$ that name the knowledge matrix $M(W{,}f)$)}, and is
...
(Proposition~\ref{prop:perm}). When the slope diagonal is locally constant --- the ReLU family, and our
setting throughout --- the KM equals per-class
...
$A=(d_\Psi/d_M)^2$ (Theorem~\ref{thm:pyth}), read against reference lines
