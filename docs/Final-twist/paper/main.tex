...
\input{sections/review_flags}

% ===== Main matter (one file per section) =====
\input{sections/introduction}        % §1  Introduction
\input{sections/fig1_germ_identity}  %     Figure 1
\input{sections/section_germ}        % §2  The knowledge matrix is the germ
\input{sections/section_geometry}    % §3  Distance geometry in matrix space
\input{sections/empirical_setup}     % §4  Experimental setup
\input{sections/study1_invariance}   % §5  Study 1 — invariance
\input{sections/study2_coherence}    % §6  Study 2 — coherence
\input{sections/study4_cross_arch}   % §7  Study 3 — cross-architecture
\input{sections/study3_visualization}% §8  Honest negatives
\input{sections/limitations}         % §9  Limitations
\input{sections/conclusion}          % §10 Conclusion

% ===== Appendices (one file per appendix) =====
\appendix
\input{sections/appendix_proofs}      % Appendix A  Proofs
\input{sections/appendix_engineering} % Appendix B  Computing the KM in software
%   (anchors thm:weff and the main-text forward pointers \ref{app:engineering})

% Appendix tables referenced from the studies.
\input{tables/table_coherence_max}
\input{tables/table_mechanism_pilot}
\input{tables/s3_table}

\input{sections/appendix_A_geometric} % Appendix C  Geometric supplement
...
\input{sections/vocab/appendix_vocab_I}
